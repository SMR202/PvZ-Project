...
int main()
{
	...
	if (!cherryBombCursorTexture.loadFromFile("Images/cherrybombcursor.png"))
		return 1;
	sf::Texture shovelCursorTexture;
	if (!shovelCursorTexture.loadFromFile("Images/shovel3.png"))
		return 1;
	...
	sf::Texture shovelSeedTexture;
	shovelSeedTexture.loadFromFile("Images/shovel.png");
	sf::Sprite shovelSeedSprite(shovelSeedTexture);
	shovelSeedSprite.setScale(0.65f, 0.65f);
	shovelSeedSprite.setPosition(400, 13);
		...
		while (window.pollEvent(event))
		{
			...
			if (event.type == Event::MouseButtonPressed)
			{
				if (event.mouseButton.button == Mouse::Left)
				{
					...
					else if (Mouse::getPosition(window).x >= 400 && Mouse::getPosition(window).x <= 516 && Mouse::getPosition(window).y >= 13 && Mouse::getPosition(window).y <= 138) {
						clicked = true;
						selectedSeed = 7;
						cursor.setTexture(shovelCursorTexture);
						cursor.setScale(0.7f, 0.7f);
					}
						...
						for (int i = 0; i < MAX_PLANTS; i++) {
							if (pvz.getLevel().getPlantFactory().getPlant(i) != nullptr) {
								...
								if (pvz.getLevel().getPlantFactory().getPlant(i)->getSprite().getGlobalBounds().contains(Mouse::getPosition(window).x, Mouse::getPosition(window).y)) {
									...
									shovel.use(pvz.getGrid(), mousePoint, pvz.getLevel().getPlantFactory());
								}
							}
						}
					}
